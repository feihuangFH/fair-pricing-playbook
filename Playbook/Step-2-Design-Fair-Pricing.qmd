...
In-processing builds fairness into the estimation step rather than adjusting inputs or outputs. One approach adds a penalty term to the model's objective function that increases as the chosen fairness criterion is violated. The model then trades off predictive accuracy against fairness violation during training, with a tuning parameter controlling the balance.

In-processing is particularly useful in price optimisation, where the optimiser's objective (typically maximising expected profit) can be extended to include a fairness constraint. [Case Study 2](<../Case Study 2/case_study2.html>) implements this in the pricing-rule context (PDP and PAF rules in Step 3).
