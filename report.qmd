...
subtitle: "A practical framework for Responsible AI in algorithmic pricing"
author:
  - name: "Fei Huang"
    affiliations:
...
Technical case studies provide implementation evidence. The Fair Models case study applies five anti-discrimination model designs (M0, MU, MDP, MCDP, and MC) to French motor insurance data using GLMs and XGBoost, comparing fairness and predictive accuracy across criteria. The Welfare Implications case study extends the analysis to the full pricing pipeline and evaluates five pricing regulations (P0, PA, POB, PDP, PAF) in terms of consumer welfare and firm profit by gender on a 2,000-policyholder French subsample. The Fairness Testing case study implements conditional demographic parity (CDP) and proxy discrimination (PD) audits with corrected statistical inference on Illinois auto insurance quote data, showing that classical OLS standard errors can be substantially wrong for deterministic pricing outputs.

The goal is operational clarity: how to choose a fairness criterion, map it to modeling interventions, evaluate market consequences, and run statistically valid compliance audits. The full playbook, including interactive case studies, implementation checklists, and reproducible code, is available at [fair.feihuang.org](https://fair.feihuang.org). All source files are openly available at [github.com/feihuangFH/fair-pricing-playbook](https://github.com/feihuangFH/fair-pricing-playbook) under CC BY 4.0.
...
@krafcheck2026fairnessmetrics extend this taxonomy to life insurance, surveying a broader set of individual and group fairness criteria and mapping them to the independence, separation, and sufficiency frameworks from the algorithmic fairness literature. Individual and group fairness criteria are often mutually incompatible. Satisfying one typically requires violating another.
...
Algorithmic discrimination (biased outcomes from analytics or machine learning) is generally a form of indirect discrimination. Omitting protected variables from a model (fairness through unawareness (FTU)) does not guarantee fair outputs if proxies or complex algorithms remain [@lindholm-etal-2022-discrimination; @xin2024antidiscrimination].
...
| Proxy discrimination (PD) [@prince2019proxy] | A non-protected variable acts as a stand-in for a protected attribute |
...
| In-processing | Enforce fairness constraints during model training or optimisation (e.g. regularisation or penalty terms in the objective). See the Welfare Implications case study for penalty-based constraints in price optimisation |
...
Protected attributes are used in training only and averaged out at prediction, giving better proxy control than MU alone [@lindholm-etal-2022-discrimination; @xin2024antidiscrimination].
...
A fair cost model does not guarantee a fair final price or favourable welfare outcomes. The Welfare Implications case study covers demand, price optimisation, and regulation comparison on insurance data.
...
@huang2026welfareaccountable highlight a persistent tension between price fairness and markup fairness. No single pricing rule achieves both simultaneously. PDP moves premiums closest to equality across protected groups but can widen markup disparities for the group the rule is meant to protect: the female-to-male markup ratio rises from 1.21 under unconstrained pricing to 1.66 under PDP. PA reduces markup gaps (ratio falls to 1.07) but at the largest profit cost of any rule, around 42 per cent. The welfare effects of POB depend on market structure, including whether participation is mandatory or voluntary and how competitive the market is.

@huang2026welfarealgorithms find that when fairness rules apply industry-wide rather than to a single firm, welfare consequences for protected groups are substantially amplified. Female consumers can no longer escape regulation-induced price distortions by switching to an unregulated competitor. Under a fairness-unawareness rule, female welfare falls by $13.43 when only one firm is regulated but by $101.67 when all insurers face the same constraint — a nearly eightfold increase. Regulators considering mandatory industry-wide mandates should account for this amplification.
...
The Fairness Testing case study implements conditional demographic parity (CDP) and proxy discrimination (PD) audits with corrected inference.
...
| TOST [@schuirmann-1987-comparison] | Equivalence testing, pass only if evidence shows the disparity is within tolerance (used for CDP) |
...
Conventional significance testing asks whether a disparity is detectably different from zero. That is a poor fit for compliance auditing [@huang2026fairtesting].
...
The protocol above tests whether premiums or prices meet quantitative fairness criteria such as CDP or proxy discrimination after legitimate controls. That is the primary audit path in this playbook and in the Fairness Testing case study.
...
The Fair Models case study uses the `pg15training` dataset from the CASdatasets R package [@dutang2020package], which is publicly available on CRAN. The Fairness Testing case study uses the ProPublica Illinois auto insurance dataset, available at http://propublica.s3.amazonaws.com/projects/carinsurance/car-insurance-public.zip. The Welfare Implications case study uses pre-computed optimization outputs derived from the French motor insurance data, and the optimization code (Python/TensorFlow) is available in the companion repository.
